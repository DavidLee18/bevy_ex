...
use crate::{game::ui::{PRESSED_BUTTON_COLOR, HOVERED_BUTTON_COLOR, NORMAL_BUTTON_COLOR, game_over_menu::components::{MainMenuButton, QuitButton, RestartButton}}, AppState};


pub fn interact_with_restart_button(mut button_query: Query<
    ...
    if let Ok((interaction, mut bg_color)) = button_query.get_single_mut() {
        on_click(interaction, &mut bg_color, &mut next_state, AppState::Game);
    }
}
...
fn on_click<T: States>(
    interaction: &Interaction,
    background_color: &mut BackgroundColor,
    next_state: &mut ResMut<NextState<T>>,
    state: T) {
    match *interaction {
        Interaction::Clicked => {
